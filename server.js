...
function createComment(url, request) {
  ...
  if (requestComment && requestComment.body &&database.articles[requestComment.articleId] && database.users[requestComment.username]) {
    const comment = {
...
function updateComment(url, request) {
  ...
  if (!commentId || !requestComment) {
    response.status = 400;
  } else if (!comment) {
    response.status = 404;
  } else {
    comment.body = requestComment.body || comment.body;
...
function upvoteComment(url, request) {
  const id = Number(url.split('/').filter(segment => segment)[1]);
  const username = request.body && request.body.username;
  let updatedComment = database.comments[id];
  const response = {};

  if (updatedComment && database.users[username]) {
    updatedComment = upvote(updatedComment, username);
...
function downvoteComment(url, request) {
  ...
  if (updatedComment && database.users[username]) {
    updatedComment = downvote(updatedComment, username);
